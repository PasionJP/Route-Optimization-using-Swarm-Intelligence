...
class ExecutePSO:
	def runPSO(edges):
		...
		graph = Graph(amount_vertices=max_value+1)

		for i in range(len(edges)):
			graph.addEdge(edges[i][0], edges[i][1], edges[i][2])

		# creates a PSO instance
		pso = PSO(graph, iterations=100, size_population=10, beta=1, alfa=1)
		pso.run() # runs the PSO algorithm
		pso.showsParticles() # shows the particles

		# shows the global best particle
		end = timer()
		# psoTime = time.time() - start_time
		psoTime = end - start
		print("--- %s seconds ---" % psoTime)
		routeArrangement = ExecutePSO.create_cycle_queue(pso.getGBest().getPBest())
		print('PSO:', [(routeArrangement, pso.getGBest().getCostPBest())])
		
		return [routeArrangement, pso.getGBest().getCostPBest()], psoTime
		'''
		# random graph
		print('Random graph...')
