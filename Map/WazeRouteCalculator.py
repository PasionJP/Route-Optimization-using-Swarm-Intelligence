...
class WazeRouteCalculator(object):
    ...
    def     calc_route_info(self, real_time=True, stop_at_bounds=False, time_delta=0):
        ...
        for item in route['results']:
            latLong = []
            latLong.append(item["path"]["y"])
            latLong.append(item["path"]["x"])
            route_path.append(latLong)
        
        route_details = []

        startToDestination = [self.start_coords['lat'], self.start_coords['lon'], self.end_coords['lat'], self.end_coords['lon'], route_time, route_distance]
        route_details.append(startToDestination)
        route_details.append(route_path)
